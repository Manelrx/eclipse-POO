			...
			if (escolha == 2 || escolha == 3 || escolha == 4) {
				...
				for (int i = 0; contas.size() > i; i++) {
					if (contas.get(i).getNumConta() == numConta) {
						...
						if (escolha == 2) {
							System.out.println(autentica.getTitular().getNome() + " Seu saldo é R$" + autentica.getSaldo());
						} else if (escolha == 3) {
							...
							if (verifica == true) {
								...
								System.out.println(autentica.getTitular().getNome() + " você não pode sacar pois seu saldo é "
										+ autentica.getSaldo());
							}
						} else if (escolha == 4) {
							System.out.print("Informe o valor que deseja depositar: ");
							contas.get(i).depositar(scan.nextDouble());
							System.out.println(autentica.getTitular().getNome() + " Seu saldo é R$" + contas.get(i).getSaldo());
						}
					}
				}
